...
$source_dir = Read-Host "`nPath to directory to make into ISO"
if((test-path $source_dir) -eq $false) {
    do {
        Write-Host "`nInvalid path! Make sure your folder exists and path is correct!`n"
        $source_dir = (Read-Host "Provide path to directory you want to make an ISO")
    } until ((test-path $source_dir) -eq $true)
}

Write-Host "`nPath to directory to SAVE the ISO to " -NoNewline
Write-Host -ForegroundColor Yellow "(Default = \\fshill\hedc\depot\tmp): " -NoNewLine
if (!($destination_dir = Read-Host )) { $destination_dir = "\\fshill\hedc\depot\tmp" }
if((test-path $destination_dir) -eq $false) {
    do {
        Write-Host "`nInvalid path! Make sure your folder exists and path is correct!`n"
        $destination_dir = (Read-Host "Provide Path to directory where you want to SAVE the ISO")
    } until ((test-path $destination_dir) -eq $true)
}
$destination_file = Read-Host "`nProvide Filename you want (ex. MySuperFunISO, exclude the .iso extension)"

$myiso = "$destination_dir\$destination_file" + ".iso"
Write-Host -ForegroundColor Green "`nCreating new ISO at $myiso" -NoNewline

get-childitem "$source_dir" | New-IsoFile -path "$myiso"