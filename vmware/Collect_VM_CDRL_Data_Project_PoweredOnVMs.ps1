...
$vmList = Get-VM -Location (Get-Folder -Name $projectName) | Where-Object {$_.PowerState -eq "PoweredOn"} | Sort-Object -Property ID
...
$startRange = $worksheet.Range("A4:AF4")
...
foreach ($vmName in $vmList) {
    ...
    $worksheet.Range("A$($startRow):AF$($startRow)").PasteSpecial(-4122) | Out-Null
    $worksheet.Range("A$($startRow):AF$($startRow)").EntireColumn.AutoFit() | Out-Null
